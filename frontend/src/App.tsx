...
interface LocalizedText{en:string;es:string}interface SiteConfigSnapshot{websiteName?:LocalizedText;browserTitle?:LocalizedText;designerName?:LocalizedText;designerTitle?:LocalizedText}const pickLocalized=(value:LocalizedText|undefined,language:"en"|"es",fallback:string)=>String((language==="es"?value?.es:value?.en)||value?.en||fallback);
function App(){const{language}=useLanguage();const[ready,setReady]=useState(false);useEffect(()=>{let cancelled=false;apiRequest<{status:string;settings:{websiteName:string;browserTitle:string;config:SiteConfigSnapshot}}>(`/api/settings?app_refresh=${Date.now()}`,{cache:"no-store"}).then(({settings})=>{if(cancelled)return;localStorage.setItem("mtd_site_config",JSON.stringify(settings.config));document.title=pickLocalized(settings.config.browserTitle,language,settings.browserTitle);const dictionaries=translations as any;const year=new Date().getFullYear();for(const lang of ["en","es"] as const){if(dictionaries[lang]?.footer){dictionaries[lang].footer.copyright=`© ${year} ${pickLocalized(settings.config.websiteName,lang,settings.websiteName)}. All rights reserved.`;const cfg=settings.config;dictionaries[lang].footer.designer=lang==="es"?`Diseñado por ${pickLocalized(cfg?.designerName,"es","J.Q")} - ${pickLocalized(cfg?.designerTitle,"es","Webmaster")}`:`Designed by ${pickLocalized(cfg?.designerName,"en","J.Q")} - ${pickLocalized(cfg?.designerTitle,"en","Webmaster")}`}}}).catch(()=>{}).finally(()=>{if(!cancelled)setReady(true)});return()=>{cancelled=true}},[language]);if(!ready)return null;return <BrowserRouter><Routes>
<Route path="/" element={<HomePage/>}/><Route path="/login" element={<LoginPage/>}/><Route path="/admin-login" element={<AdminLogin/>}/><Route path="/register" element={<RegisterPage/>}/><Route path="/forgot-password" element={<ForgotPasswordPage/>}/><Route path="/reset-password" element={<ResetPasswordPage/>}/>
