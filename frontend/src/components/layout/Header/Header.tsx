...
interface SettingsData {
    ...
    browserTitle: string;
    logoUrl: string | null;
    config?: { websiteName?: LocalizedText; browserTitle?: LocalizedText };
}
...
const formatBrandName = (websiteName: string): { title: string; subtitle: string } => {
    ...
    return { title: words.slice(0, -1).join(" ").toUpperCase(), subtitle: words[words.length - 1].toUpperCase() };
};

const localized = (value: LocalizedText | undefined, language: "en" | "es", fallback: string) => {
...
function Header() {
    ...
    const getUserInitials = (user: AuthenticatedUser) => {
        const initials = `${user.first_name?.trim().charAt(0).toUpperCase() || ""}${user.last_name?.trim().charAt(0).toUpperCase() || ""}`;
    ...
    useEffect(() => {
        const loadSettings = async () => {
            try {
                const result = await apiRequest<{ status: string; settings: SettingsData }>(`/api/settings?public_refresh=${Date.now()}`, { cache: "no-store" });
                if (result.settings?.websiteName) setWebsiteName(result.settings.websiteName);
                setLocalizedWebsiteName(result.settings?.config?.websiteName);
                setLocalizedBrowserTitle(result.settings?.config?.browserTitle);
    ...
    useEffect(() => {
        const title = localized(localizedBrowserTitle, language, "Magic Touch Designs | Personalized Gifts & Designs");
        document.title = title;
    }, [language, localizedBrowserTitle]);

    useEffect(() => {
        const updateCartCount = () => setCartCount(getCartItems().reduce((sum, item) => sum + item.quantity, 0));
        updateCartCount();
    ...
    const toggleAccountMenu = () => { if (!currentUser) { navigate("/login"); return; } setAccountMenuOpen(previous => !previous); };
    ...
    const handleLogout = () => { localStorage.removeItem("auth_token"); localStorage.removeItem("auth_user"); setCurrentUser(null); setAccountMenuOpen(false); navigate("/login"); };
            ...
            <div className="header__container">
                ...
                <div className="header__actions">
                    ...
                    <div className="header__account">
                        <button type="button" className={`header__account-trigger ${currentUser ? "header__account-trigger--authenticated" : ""}`} aria-label={currentUser ? `${language === "es" ? "Cuenta" : "Account"}: ${currentUser.username}` : language === "es" ? "Cuenta" : "Account"} aria-expanded={accountMenuOpen} onClick={toggleAccountMenu}><span className="header__account-icon"><User size={20} /></span>{currentUser && <span className="header__username">{isMobileView ? getUserInitials(currentUser) : currentUser.username}</span>}{currentUser && <ChevronDown size={15} className={`header__account-chevron ${accountMenuOpen ? "header__account-chevron--open" : ""}`} />}</button>
