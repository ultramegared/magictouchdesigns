...
const read = (): any => {
    try {
        ...
        return {shop:sections[0]||fallback.shop,company:sections[1]||fallback.company,support,social:(c.socialLinks||[]).filter((x:any)=>x.active).sort((a:any,b:any)=>a.order-b.order).map((x:any)=>({name:x.name,href:x.url})),phone:c.businessPhone||fallback.phone,copyright:fallback.copyright,designer:fallback.designer};
