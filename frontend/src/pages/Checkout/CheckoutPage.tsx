...
const loadScript = (id: string, src: string) =>
    new Promise<void>((resolve, reject) => {
        ...
        s.onerror = () =>
            reject(
                new Error(
                    "Secure payment provider could not be loaded.",
                ),
            );
...
function CheckoutPage() {
    ...
    const valid = () =>
        Boolean(formRef.current?.reportValidity());

    const payload = () => ({
        customer: form,

        items: cartItems.map((item) => {
            const session = item.customizationId
            ...
            if (
                item.customizationId &&
                (!session?.designDataUrl ||
                    session.productId !== String(item.id))
            ) {
                throw new Error(
                    "Your custom design session expired. Please return to Customize and upload the artwork again.",
                );
            }
    ...
    const prepareStripe = async () => {
        ...
        try {
            ...
            if (actions.type !== "success") {
                throw new Error(
                    actions.error.message ||
                        "Stripe checkout could not initialize.",
                );
            }

            stripeActionsRef.current =
                actions.actions;

            stripeSessionRef.current = {
                id: d.sessionId,
                code: d.orderCode,
            };
            ...
            const express =
                checkout.createExpressCheckoutElement({
                    buttonHeight: 52,
            ...
            if (
                !stripePaymentRef.current ||
                !stripeAppleRef.current
            ) {
                throw new Error(
                    "Payment area is unavailable.",
                );
            }
                        ...
                        if (
                            ...
                            setError(
                                result.error?.message ||
                                    "Apple Pay payment could not be completed.",
                            );
            ...
            setError(
                x instanceof Error
                    ? x.message
                    : "Unable to load secure card payment.",
            );
        } finally {
            setLoading(false);
        }
    ...
    const submit = async (
        ...
        if (!actions) {
            return setError(
                "Secure card payment is not ready.",
            );
        }
            ...
            if (r?.type === "error") {
                ...
                window.location.assign(
                    `/checkout/success?session_id=${encodeURIComponent(
                        stripeSessionRef.current.id,
                    )}&order_code=${encodeURIComponent(
                        stripeSessionRef.current.code,
                    )}`,
                );
            }
        } catch (x) {
                    ...
                    sdk.createPayPalOneTimePaymentSession(
                        {
                            onApprove: async ({
                                ...
                                const r =
                                    await fetch(
                                        `${API_URL}/orders/paypal/${encodeURIComponent(
                                            orderId,
                                        )}/capture`,
                                        {
                                            method: "POST",
                                ...
                                window.location.assign(
                                    `/checkout/success?paypal=1&order_code=${encodeURIComponent(
                                        d.orderCode,
                                    )}`,
                                );
                            },
                    ...
                    >
                        ...
                        <aside className="checkout-summary">
                            ...
                            <p className="checkout-summary__note">
                                ...
                                calculated from the shipping
                                destination. Shipping is
