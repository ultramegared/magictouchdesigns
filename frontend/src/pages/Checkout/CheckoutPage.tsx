...
function CheckoutPage() {
    ...
    const payload = () => ({ customer: form, items: cartItems.map((item) => {
        ...
        return { productId: String(item.id), quantity: item.quantity, model: item.model, size: item.size, color: item.color,
    ...
    useEffect(() => { if ((method === "card" || method === "apple") && cartItems.length && addressReady) void prepareStripe(); }, [method, cartItems.length, addressReady]);
