...
function AdminSales(){
 ...
 const hasDailySales=(data?.daily||[]).some(d=>Number(d.revenue||0)>0);
 return <div className="admin-page"><AdminSidebar/><main className="admin-sales">
  ...
  {loading&&!data?<div className="admin-sales__loading">Loading sales performance…</div>:<>
   ...
   <section className="admin-sales__grid admin-sales__grid--wide"><article className="admin-sales__card"><div className="card-title"><div><span>SALES PULSE</span><h2>Revenue by day</h2></div><b>{money(revenue)}</b></div>{hasDailySales?<div className="sales-bars">{(data?.daily||[]).map(d=><div className="sales-bar" key={d.date} title={`${d.date}: ${money(Number(d.revenue))}`}><i style={{height:`${Math.max(4,(Number(d.revenue)/maxDaily)*100)}%`}}/><small>{d.date.slice(5)}</small></div>)}</div>:<div className="sales-bars--empty"><span>No paid product revenue was recorded in the selected period.</span></div>}<p className="note">Only paid order product revenue is shown here. Reports is the source for formal historical reporting.</p></article>
