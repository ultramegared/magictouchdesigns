...
const getPayPalBaseUrl = (): string => {
    if (process.env.PAYPAL_BASE_URL) return process.env.PAYPAL_BASE_URL.replace(/\/+$/, "");
...
const paypalJsonRequest = async (path: string, method: "GET" | "POST", token: string, body?: unknown): Promise<any> => {
    const response = await fetch(`${getPayPalBaseUrl()}${path}`, { method, headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", Accept: "application/json", "PayPal-Request-Id": crypto.randomUUID() }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) });
    const data = await response.json() as any;
    if (!response.ok) throw new Error(data?.message || data?.details?.[0]?.description || "PayPal request failed.");
    return data;
};
...
const getPayPalAccessToken = async (): Promise<string> => {
    ...
    const response = await fetch(`${getPayPalBaseUrl()}/v1/oauth2/token`, { method: "POST", headers: { Authorization: `Basic ${credentials}`, "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" }, body });
    const data = await response.json() as any;
    if (!response.ok || !data?.access_token) throw new Error(data?.error_description || "Unable to authenticate with PayPal.");
    return String(data.access_token);
};

export const getPayPalBrowserClientToken = async (): Promise<string> => {
    const { clientId, secret } = requirePayPalCredentials();
    const credentials = Buffer.from(`${clientId}:${secret}`).toString("base64");
    const body = new URLSearchParams({ grant_type: "client_credentials", response_type: "client_token", "domains[]": "jqydesigns.com" });
    const response = await fetch(`${getPayPalBaseUrl()}/v1/oauth2/token`, { method: "POST", headers: { Authorization: `Basic ${credentials}`, "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" }, body });
    const data = await response.json() as any;
    if (!response.ok || !data?.access_token) throw new Error(data?.error_description || data?.message || "Unable to initialize PayPal securely.");
    return String(data.access_token);
};
