...
export const getAdminReports = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const [summary, daily, payments, products, monthly, leads, recentLeads, customers, transactions] = await Promise.all([
            pool.query(`
                SELECT
                    COUNT(*)::int AS orders,
            ...
            pool.query(`
                ...
                WHERE ${rangeWhere}
                GROUP BY 1, 2
                HAVING COUNT(*) FILTER (WHERE payment_status = 'paid') > 0
                ORDER BY sales DESC
            `, values),
            pool.query(`
                SELECT
                ...
                WHERE o.${rangeWhere.replaceAll("created_at", "created_at")}
            ...
            `, values),
            pool.query(`
                SELECT id, email, is_active, language, created_at
                FROM subscribers
                WHERE created_at >= $1::date
                  AND created_at < ($2::date + INTERVAL '1 day')
            ...
            pool.query(`
                ...
                FROM paid_customers
                WHERE email IN (
                    SELECT DISTINCT LOWER(customer_email)
                    FROM orders
                    WHERE ${paidRangeWhere}
                )
            `, values),
            pool.query(`
                SELECT
        ...
        res.json({
            ...
            leads: {
                ...leads.rows[0],
                note: "Leads are newsletter subscribers recorded in the database. Lead source and lead-to-order attribution are not currently tracked.",
            },
